...
---
### 多线程
进程是 是具有一定独立功能的程序关于某个数据集合上的一次运行活动,是并发执行的程序在执行过程中分配和管理资源的基本单位. 
...
### 数据库
#### 范式
1. 第一范式（1NF） 某个属性不能有多个值或者不能有重复的属性
2. 第二范式（2NF） 要求数据库表中的每个实例或行必须可以被惟一地区分
3. 第二范式（2NF） 要求一个数据库表中不包含已在其它表中已包含的非主关键字信息
4. 鲍依斯-科得范式（BCNF）数据库表中如果不存在任何字段对任一候选关键字段的传递函数依赖则符合第三范式

### 内存
#### 基本的管理策略
* 你拥有你创健的对象,创建的对象（使用alloc，new，copy或者mutalbeCopy等方法）的初始引用计数是1,
